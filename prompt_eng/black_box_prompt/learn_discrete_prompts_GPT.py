...
LABEL2ID_CONFIG = {"visit": 0, "sequenced_visit": 1, "ordered_visit": 2, "strictly_ordered_visit": 3, "patrolling": 4}
LABEL_CONVERT = {0: "visit", 1: "sequenced_visit", 2: "ordered_visit", 3: "strictly_ordered_visit", 4: "patrolling"}
...
def parse_args():
    ...
    parser.add_argument("--model_name_or_path", type=str, default='text-davinci-003', help="Path to pretrained model or model identifier from huggingface.co/models.")
    ...
    parser.add_argument("--seed", type=int, default=484, help="A seed for reproducible training.")
    ...
    args.train_file = "train_set.csv"
    args.validation_file = "validation_set.csv"
    args.test_file = "test_set.csv"
...
def pmi():
    args = parse_args()
    result=[]
    with open("./pmi/pmi_ltl_classification_gpt.txt",'r') as f:
        for line in f:
...
def main():
    ...
    # label_to_id = None
    label_to_id = LABEL2ID_CONFIG
    id_to_label = LABEL_CONVERT
    ...
    # we need a key to represent our task. Since the input is a question, we call it "question". The label gets no key, so we have None
    sentence1_key = "instruction"
    sentence2_key = None
    ...
    def preprocess_function(examples):
        ...
        # this is only for manual prompting zero-shot
        # if args.task_name is not None:
        ...
        for i in range(len(examples[sentence1_key])):
            if sentence2_key is None:
                ...
                result["input"].append('input: '+ new_sent + " What is the linear temporal logic formula type?" + "\n" + "output:")

        # labels needs to be words
    ...
    if os.path.isfile("prompts_probs.pt"):
        # Load prompts_probs
        prompts_probs = torch.load("prompts_probs.pt")
        print("Prompts_probs loaded successfully!")
    else:
    ...
    for epoch in range(args.num_train_epochs):
        ...
        try:
            for step in range(len(train_batches['sentence'])):
                print("length of train_batches: ", len(train_batches['sentence']))
                ...
                with torch.no_grad():
                    ...
                    if step % 20 == 0:
                        torch.save(prompts_probs, "prompts_probs.pt")
        ...
        if eval_result > best_eval_result:
            ...
            torch.save(best_prompts_probs, "best_prompts_probs.pt")
