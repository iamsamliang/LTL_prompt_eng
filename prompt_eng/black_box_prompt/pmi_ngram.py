...
# should this be same tokenizer as the one in learn_discrete_prompts_GPT
...
# should only be the training dataset, not entire dataset
dataset = load_from_file("train_set.csv")
# dataset = load_from_file("data/symbolic_batch1_noperm.csv")
# f_read = open(config.dataset, 'r')
config.output_dir = "./pmi/pmi_ltl_classification_gpt.txt"
if not os.path.exists("pmi"):
    os.makedirs(f"pmi/")
f_write = open(config.output_dir, 'w')

sentence_list = []
# for _, _, instruction, _ in dataset:
#     sentence_list.append(instruction)
...
for ngram_phrase in ngram_list:
    ngram_index = tokenizer.encode_ordinary(' ' + ngram_phrase)
